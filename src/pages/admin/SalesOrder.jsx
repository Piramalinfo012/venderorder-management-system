...
const SO_FORM_URL =
  "https://script.google.com/macros/s/AKfycbzf6N6VaqepWzeyZZ02R76hcvq300bF340c_w-GswRdcPsOTH4zUraxT_c9WwkeDlLE/exec";

const SHEET_ID = "1en1XvWXKU86YlIgFmPl_b5vChuW9xeVWbYyyG_kpd9Y";
...
const normalizeSOs = (rows) =>
  rows
    .map((row, index) => {
      // Find PDF Edit URL (starts with docs.google.com)
      const pdfEditUrl = row.find(val => typeof val === 'string' && val.includes('docs.google.com/document')) || "";
      
      // Find PDF URL (starts with drive.google.com)
      const pdfUrl = row.find(val => typeof val === 'string' && val.includes('drive.google.com/file')) || "";

      const mapped = SO_COLUMNS.reduce((acc, key, colIdx) => {
        acc[key] = row[colIdx] ?? "";
        return acc;
      }, {});
      
      // Override with dynamically found URLs
      if (pdfEditUrl) mapped.pdfEditUrl = pdfEditUrl;
      if (pdfUrl) mapped.pdfUrl = pdfUrl;

      return { id: `so-${index}`, ...mapped };
    })
    .filter((so) => so.orderNo || so.partyName || so.date);
...
const SalesOrder = () => {
  ...
  useEffect(() => {
    const fetchOrders = async () => {
      const cacheKey = "vendor-admin:so-data";
      ...
      try {
        ...
        const sheetName = encodeURIComponent("SO");
        const url = `${SCRIPT_URL}?sheet=${sheetName}&id=${SHEET_ID}&spreadsheetId=${SHEET_ID}&t=${Date.now()}`;
        console.log("Fetching SO from:", url);
        ...
        if (!Array.isArray(data)) {
          throw new Error(result.error || "Sales Order data unavailable — check sheet name in Google Script");
        }
        
        if (data.length > 1) {
          setDebugInfo(`Raw Row 1 (${data[1].length} cols): ${JSON.stringify(data[1])}`);
        }
        
        const rows = normalizeSOs(data.slice(1)).reverse();
  ...
  const filtered = useMemo(() => {
    ...
    return orders.filter((so) =>
      [so.orderNo, so.partyName, so.productName, so.date]
        .filter(Boolean)
  ...
  return (
    <div className="min-h-screen bg-[radial-gradient(circle_at_top_left,_#ddd6fe,_#f5f3ff_25%,_#f8fafc_55%,_#ede9fe_100%)] px-2 py-4 sm:px-6 sm:py-5 sm:px-6 lg:px-8">
      <div className="mx-auto max-w-7xl space-y-6">
        ...
        {/* Hero Header */}
        <section className="overflow-hidden rounded-[1.25rem] sm:rounded-[2rem] bg-[linear-gradient(135deg,#020617,#4c1d95_40%,#6d28d9_100%)] text-white shadow-[0_30px_80px_-35px_rgba(109,40,217,0.7)]">
          <div className="grid gap-4 sm:gap-6 px-2 py-4 sm:px-6 sm:py-5 sm:px-8 lg:grid-cols-[1.4fr_0.6fr]">
            ...
            <div className="space-y-5">
              <div className="space-y-2">
                ...
                <p className="text-sm leading-7 text-violet-100">
                  Live sales order records fetched directly from your Google Sheet.
                </p>
              </div>

              <div className="grid grid-cols-3 gap-4">
                <div className="rounded-2xl border border-white/10 bg-white/10 p-4">
                  <p className="text-xs uppercase tracking-[0.18em] text-violet-200">Total SOs</p>
            ...
            <div className="flex flex-col gap-4 rounded-[1.75rem] border border-white/10 bg-white/10 p-5 backdrop-blur-sm">
              ...
              <button
                onClick={() => window.open(SO_FORM_URL, "_blank")}
              ...
              >
                ...
                <div>
                  <p className="font-bold text-sm">New Sales Order</p>
                  <p className="text-xs text-slate-500 mt-0.5">Open SO form in new tab</p>
                </div>
          ...
          {filtered.length === 0 ? (
            <div className="mt-6 rounded-[1.5rem] border border-dashed border-slate-200 bg-slate-50 p-10 text-center">
              <p className="text-sm text-slate-500">
                {orders.length === 0
                  ? "No sales orders found in the sheet."
            ...
            <div className="mt-5 grid grid-cols-1 gap-4 lg:grid-cols-2 xl:grid-cols-3">
              {filtered.map((so) => (
                <div
                  key={so.id}
                  className="group rounded-[1.25rem] sm:rounded-[1.5rem] border-2 border-slate-200 bg-[linear-gradient(180deg,#ffffff,#f8fafc)] p-3 sm:p-5 shadow-md transition hover:-translate-y-0.5 hover:border-violet-300 hover:shadow-lg hover:shadow-violet-200/60"
                >
                  {/* Card Header */}
                  ...
                  <div className="mt-4 space-y-2 text-sm">
                    ...
                    <div className="flex items-center gap-2 text-slate-600">
                      ...
                      <span className="truncate">{so.productName || "—"}</span>
                    </div>
                    <div className="flex items-center gap-2 text-slate-600">
